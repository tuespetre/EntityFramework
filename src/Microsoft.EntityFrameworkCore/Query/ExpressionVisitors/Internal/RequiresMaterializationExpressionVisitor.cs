    ...
    public class RequiresMaterializationExpressionVisitor : ExpressionVisitorBase
    {
        ...
        private void HandleUnderlyingQuerySources(IQuerySource querySource, Action<IQuerySource> action)
        {
            ...
            else
            {
                ...
                {
                    var lastResultOperator = subQueryExpression.QueryModel.ResultOperators.LastOrDefault();

                    if (lastResultOperator is IQuerySource resultOperatorQuerySource)
                    {
                        action(resultOperatorQuerySource);
                    }
