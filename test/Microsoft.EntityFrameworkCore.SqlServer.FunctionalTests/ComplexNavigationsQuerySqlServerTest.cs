...
) AS [t] ON [l].[Id] = [t].[OneToMany_Optional_InverseId]
...
WHERE EXISTS (
    ...
    WHERE [t].[Id] IS NOT NULL AND (([l0].[OneToMany_Optional_InverseId] = [l].[Id]) AND [l0].[OneToMany_Optional_InverseId] IS NOT NULL))
ORDER BY [l0].[OneToMany_Optional_InverseId]",
                Sql);
        }
...
LEFT JOIN (
    ...
    ORDER BY [x.OneToOne_Optional_FK].[Name], [x].[Id]
